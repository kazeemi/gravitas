...
10. confidenceLanguageObservation — Quote any hedging language heard ("I think", "maybe", "kind of", "sort of", "I'm not sure but", "hopefully"), any clear declarative/confident language ("I recommend", "We will", "The evidence shows"), and any aggressive or demeaning language (name-calling, inflammatory phrases). For each, quote the exact words heard. 2-4 sentences. Write "no notable confidence-language signal" if the language was neutral throughout.
   CULTURAL/RELIGIOUS EXPRESSIONS — NEVER FLAG: Common religious or cultural expressions (e.g. "God willing"/"InshaAllah", "if God wills it", "MashaAllah", "Bismillah", "Alhamdulillah", "Wallah", "Subhanallah", and equivalents in any language) are routine speech conventions, not hedging, uncertainty, or a signal the speaker feels out of control. Do not quote, flag, or otherwise reference these phrases in confidenceLanguageObservation under any framing — treat them exactly as you would a neutral conversational filler like "so" or "well", i.e. invisible to this assessment.
...
${input.wpmWindows && input.wpmWindows.length > 0 ? `You have access to per-5-second WPM data in Source B. Use it to identify localised pace changes within the session. Do NOT flag every deviation — only flag a pace change when there is evidence it affected the listening experience.
...
- GOVERNING PRINCIPLE: Pace variation is not inherently good or bad. Reward deliberate variation. Flag variation that worked against the message. Ignore variation that had no meaningful impact either way.` : `NO WINDOWED PACE DATA AVAILABLE for this session (the transcription model used for this language does not return word-level timestamps) — you only have the single overall WPM figure in Source B. Do NOT invent a specific moment, timestamp, or localised rush/slowdown you have no data for. Score and write the pace dimension using only the overall WPM versus the ideal range, and SOURCE A's general delivery observations if they describe a rush or slowdown qualitatively (e.g. "sped up noticeably toward the end") — in that case you may reference it in general terms, but do not attach a timestamp or window-based number to it.`}

PAUSING:
${input.pauseMetrics != null ? `MOMENT-LEVEL FEEDBACK REQUIRED — real pause-timing data is available in SOURCE B (pause count, durations, and whether each fell at a sentence boundary or mid-sentence). Do not just score the pattern. Always name specific moments and their impact, grounded in that data.
...
- Never give only a pattern-level summary. Always name the moment and the impact.` : `NO PAUSE-TIMING DATA AVAILABLE for this session (the transcription model used for this language does not return word-level timestamps). You do NOT have the information needed to name a specific pause, its duration, or where it fell — do not invent one. Score and write this dimension using only a general impression from what SOURCE A's delivery observations describe about rhythm and flow (if anything), and phrase feedback in general terms (e.g. overall use of pausing to create emphasis, or a general tendency to run sentences together) rather than citing a specific moment, timestamp, or duration that was not actually measured.`}
...
CULTURAL/RELIGIOUS EXPRESSIONS — NEVER PENALISE, NEVER INTERPRET: Common religious or cultural expressions — "God willing"/"InshaAllah", "if God wills it", "MashaAllah", "Bismillah", "Alhamdulillah", "Wallah", "Subhanallah", and equivalents in any language — are routine speech conventions across many cultures, not hedging and not a signal about the speaker's sense of control, ownership, or confidence. Even if such a phrase appears in confidenceLanguageObservation, do not classify it as hedging, do not draw any inference from it ("signals results are outside their control", "weakens their image as being in control", or similar), and do not mention it anywhere in the feedback at all. Treat it as if it were not said. This is a strict rule with no exceptions — getting this wrong is a serious cultural-sensitivity failure, not a minor scoring error.
