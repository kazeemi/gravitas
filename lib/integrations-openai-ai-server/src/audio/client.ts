...
function getClient(): OpenAI {
  ...
  return new OpenAI({
    ...
    // Same reasoning as the Anthropic client: widen the SDK's built-in
    // retry-with-backoff (connection errors, 429, 5xx) past its default of 2
    // attempts, since concurrent transcription/delivery-analysis calls make
    // hitting OpenAI's rate limits more likely under load.
